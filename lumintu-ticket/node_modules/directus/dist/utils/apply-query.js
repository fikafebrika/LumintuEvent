...
function applyQuery(knex, collection, dbQuery, query, schema, subQuery = false) {
    if (query.sort) {
        dbQuery.orderBy(query.sort.map((sort) => ({
            ...sort,
            column: (0, get_column_1.getColumn)(knex, collection, sort.column, false),
        })));
    }
            ...
            function followRelation(pathParts, parentCollection = collection, parentAlias) {
                ...
                if (relationType === 'm2a') {
                    ...
                    dbQuery.leftJoin({ [alias]: pathScope }, (joinClause) => {
                        joinClause
                            .on(`${parentAlias || parentCollection}.${relation.field}`, '=', `${alias}.${schema.collections[pathScope].primary}`)
